...
import { type Extra, extras as extrasModel } from "@/models/extras";
...
export class Trick {
  ...
  checkForAffinityEvent(card: Card, player: Player): void {
    ...
    if (this.contains(queen.of(Suit.Clubs))) {
      if (card.is(ten.of(Suit.Hearts))) {
        this.players.forEach((p) => {
          p.behavior.handleAffinityEvent(AffinityEvent.QueenOfClubsTricked, player);
        });
      }
      if (card.is(ace.of(Suit.Diamonds)) || card.is(ten.of(Suit.Diamonds))) {
        this.players.forEach((p) => {
          p.behavior.handleAffinityEvent(AffinityEvent.QueenOfClubsGreased, player);
        });
      }
      ...
      if (
        ...
        this.players.forEach((p) => {
          p.behavior.handleAffinityEvent(AffinityEvent.QueenOfClubsNotGreased, player);
        });
      }
    }
  }
  ...
  cardBy(player: Player): PlayedCard {
    return this.playedCards.filter((playedCard) => playedCard.player.id === player.id)[0];
  }

  contains(card: Card): boolean {
    return this.playedCards.filter((playedCard) => playedCard.card.is(card)).length > 0;
  }
  ...
  findFox(): PlayedCard[] {
    return this.playedCards.filter(
      (playedCard) => playedCard.card.rank === Rank.Ace && playedCard.card.suit === Suit.Diamonds,
    );
  }
  ...
  findCharlie(): PlayedCard[] {
    return this.playedCards.filter(
      (playedCard) => playedCard.card.rank === Rank.Jack && playedCard.card.suit === Suit.Clubs,
    );
  }
