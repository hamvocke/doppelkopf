...
import { type Extra, extras } from '@/models/extras'
...
export class Score {
  ...
  winningPartyName(): PartyName | null {
    ...
    if (reAnnouncements.includes(Announcement.No90)) {
      ...
      kontraWinningThreshold = kontraAnnouncements.length > 0 ? kontraWinningThreshold : 90
    }

    if (reAnnouncements.includes(Announcement.No60)) {
      reWinningThreshold = 181
      kontraWinningThreshold = kontraAnnouncements.length > 0 ? kontraWinningThreshold : 60
    }

    if (reAnnouncements.includes(Announcement.No30)) {
      reWinningThreshold = 211
      kontraWinningThreshold = kontraAnnouncements.length > 0 ? kontraWinningThreshold : 30
    }

    if (reAnnouncements.includes(Announcement.NoPoints)) {
      reWinningThreshold = 240
      kontraWinningThreshold = kontraAnnouncements.length > 0 ? kontraWinningThreshold : 1
    }
  ...
  _hasAnyPartyAnnounced(announcement: Announcement) {
    return Object.values(this.parties).some((party) => party.announcements().includes(announcement))
  }
  ...
  listExtras(partyName: PartyName) {
    ...
    const opponentAnnouncements =
      this.parties[partyName == PartyName.Re ? PartyName.Kontra : PartyName.Re].announcements()
    ...
    if (partyName === this.winningPartyName()) {
      ...
      if (partyPoints >= 120 && opponentAnnouncements.includes(Announcement.No90)) {
      ...
      if (partyPoints >= 90 && opponentAnnouncements.includes(Announcement.No60)) {
      ...
      if (partyPoints >= 60 && opponentAnnouncements.includes(Announcement.No30)) {
      ...
      if (partyPoints >= 30 && opponentAnnouncements.includes(Announcement.NoPoints)) {
